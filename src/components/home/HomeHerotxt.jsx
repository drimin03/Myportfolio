import React from "react";
import Video from "./Video";

const HomeHerotxt = () => {
  return (
    <div className="text-center lg:mt-0  mt-70 font-[manrope1] mr-3.5 top-3.5">
      <div className=" uppercase lg:text-[9vw]  text-[11vw] lg:leading-[7.5vw]  leading-[10vw] ">Welcome</div>
      <div className=" uppercase lg:text-[9vw] text-[11vw] lg:leading-[7.5vw] leading-[10vw] flex items-center justify-center">to
        <div className="h-[7vw] w-[100px] lg:w-[330px] rounded-full border-1 overflow-hidden flex items-center justify-center bg-gradient-to-r from-gray-700 to-gray-900">
          <span className="text-white  text-2xl">
            <Video />
          </span>
        </div>
        my
      </div>

      <div className=" uppercase lg:text-[9vw] text-[11vw]  lg:leading-[7.5vw] leading-[10vw] ">Portfolio</div>
    </div>
  );
};

export default HomeHerotxt;
